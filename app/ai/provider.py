...
class VisionGenerationProvider(Protocol):
...
class OpenAIResponsesTextProvider:
    ...
    def _build_result(self, response: httpx.Response) -> TextGenerationResult:
        try:
        ...
        *,
        ...
        for image in images:
            if image.data is None:
                raise GeminiProviderError("Gemini image input requires binary image data")
...
class ClaudeMessagesTextProvider:
    ...
    def generate_text_from_images(
        ...
        *,
        ...
        for image in images:
            if image.data is None:
                raise ClaudeProviderError("Claude image input requires binary image data")
