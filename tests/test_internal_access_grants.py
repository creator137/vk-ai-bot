...
class InternalAccessGrantEndpointTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_issuance_creates_grant_through_vk_user_id_path(self) -> None:
        with TestClient(app) as client:
            response = client.post(
                "/internal/access-grants",
                json={"vk_user_id": 123456},
        ...
        with self.session_factory() as session:
            ...
            grants = session.execute(select(AccessGrant)).scalars().all()

        self.assertEqual(len(users), 1)
        self.assertEqual(users[0].vk_user_id, 123456)
        self.assertEqual(len(grants), 1)
        self.assertEqual(grants[0].user_id, users[0].id)

    def test_repeated_issuance_is_idempotent(self) -> None:
        with TestClient(app) as client:
        ...
        self.assertTrue(first_response.json()["grant_created"])
        self.assertFalse(second_response.json()["grant_created"])
