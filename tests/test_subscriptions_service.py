...
class SubscriptionServiceTests(unittest.TestCase):
    ...
    def test_daily_bonus_is_issued_after_tokens_are_exhausted(self) -> None:
        ...
        assert subscription is not None
        self.assertEqual(issued.included_tokens, 3_000)
        self.assertEqual(issued.used_tokens, 0)
        self.assertEqual(subscription.included_tokens, 3_000)
        self.assertEqual(subscription.used_tokens, 0)
    ...
    def test_daily_bonus_is_not_reissued_on_same_day(self) -> None:
        with self.session_factory() as session:
            ...
            subscription = service._repository.get_by_user_id(user.id)  # noqa: SLF001

        self.assertIsNotNone(first_issue)
        self.assertIsNone(second_issue)
        assert subscription is not None
        self.assertEqual(subscription.included_tokens, 3_000)
        self.assertEqual(subscription.used_tokens, 3_000)
    ...
    def test_daily_bonus_can_be_issued_again_on_next_day(self) -> None:
        ...
        self.assertTrue(decision.allowed)
        assert subscription is not None
        self.assertEqual(next_issue.included_tokens, 3_000)
        self.assertEqual(next_issue.used_tokens, 0)
        self.assertEqual(subscription.included_tokens, 3_000)
        self.assertEqual(subscription.used_tokens, 0)

    def test_daily_bonus_keeps_only_current_day_overspend_for_free_plan(self) -> None:
        with self.session_factory() as session:
            user = UserService(session).find_or_create_by_vk_user_id(123456)
            service = SubscriptionService(session=session)
            service.consume_tokens_if_present(user_id=user.id, total_tokens=5_400)
        ...
        assert subscription is not None
        self.assertEqual(issued.included_tokens, 3_000)
        self.assertEqual(issued.used_tokens, 400)
        self.assertEqual(subscription.included_tokens, 3_000)
        self.assertEqual(subscription.used_tokens, 400)
